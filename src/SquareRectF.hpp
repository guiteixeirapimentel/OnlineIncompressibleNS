...
void SquareRectF()
{
	constexpr float h = 0.02 /1.2;

	constexpr float l = 10.0*h * 1.2;

	constexpr float L = 18.0*l;
	constexpr float H = 10.0*l;
	constexpr float W = 6.0*l;
	...
	constexpr float ufree = 1.0;
	constexpr float vfree = 0.0;
	constexpr float wfree = 0.0;

	constexpr float dt = h / 10;

	constexpr float cfl = ufree * dt / h;

	constexpr float rho = 1.0;
	constexpr float mu = 0.0025;//1.5e-5;
	...
	for (int it = 0; it < NT; it++)
	{
		...
		std::thread dphidtumomThread(&SquareCylinder3DStaggeredFieldF::CalcdphidtUmomDiv, &fdcolfield, std::ref(fdcolfield.cdUdtn),
		...
		std::thread dphidtvmomThread(&SquareCylinder3DStaggeredFieldF::CalcdphidtVmomDiv, &fdcolfield, std::ref(fdcolfield.cdVdtn),
		...
		std::thread dphidtwmomThread(&SquareCylinder3DStaggeredFieldF::CalcdphidtWmomDiv, &fdcolfield, std::ref(fdcolfield.cdWdtn),
		...
		std::thread dphidtumomThread1(&SquareCylinder3DStaggeredFieldF::CalcdphidtUmomDiv, &fdcolfield, std::ref(fdcolfield.cdUdt1),
			std::ref(aa), std::ref(bb), std::ref(cc));

		//fdcolfield.CalcdphidtVmom(fdcolfield.cdVdt1, aa, bb, cc);

		std::thread dphidtvmomThread1(&SquareCylinder3DStaggeredFieldF::CalcdphidtVmomDiv, &fdcolfield, std::ref(fdcolfield.cdVdt1),
			std::ref(aa1), std::ref(bb1), std::ref(cc1));

		//fdcolfield.CalcdphidtWmom(fdcolfield.cdWdt1, aa, bb, cc);

		std::thread dphidtwmomThread1(&SquareCylinder3DStaggeredFieldF::CalcdphidtWmomDiv, &fdcolfield, std::ref(fdcolfield.cdWdt1),
			std::ref(aa2), std::ref(bb2), std::ref(cc2));
		...
		std::thread dphidtumomThread2(&SquareCylinder3DStaggeredFieldF::CalcdphidtUmomDiv, &fdcolfield, std::ref(fdcolfield.cdUdt2),
			std::ref(aa), std::ref(bb), std::ref(cc));

		//fdcolfield.CalcdphidtVmom(fdcolfield.cdVdt2, aa, bb, cc);

		std::thread dphidtvmomThread2(&SquareCylinder3DStaggeredFieldF::CalcdphidtVmomDiv, &fdcolfield, std::ref(fdcolfield.cdVdt2),
			std::ref(aa1), std::ref(bb1), std::ref(cc1));

		//fdcolfield.CalcdphidtWmom(fdcolfield.cdWdt2, aa, bb, cc);

		std::thread dphidtwmomThread2(&SquareCylinder3DStaggeredFieldF::CalcdphidtWmomDiv, &fdcolfield, std::ref(fdcolfield.cdWdt2),
			std::ref(aa2), std::ref(bb2), std::ref(cc2));
		...
		std::thread dphidtumomThread3(&SquareCylinder3DStaggeredFieldF::CalcdphidtUmomDiv, &fdcolfield, std::ref(fdcolfield.cdUdt3),
			std::ref(aa), std::ref(bb), std::ref(cc));

		//fdcolfield.CalcdphidtVmom(fdcolfield.cdVdt3, aa, bb, cc);
			   
		std::thread dphidtvmomThread3(&SquareCylinder3DStaggeredFieldF::CalcdphidtVmomDiv, &fdcolfield, std::ref(fdcolfield.cdVdt3),
			std::ref(aa1), std::ref(bb1), std::ref(cc1));

		//fdcolfield.CalcdphidtWmom(fdcolfield.cdWdt3, aa, bb, cc);

		std::thread dphidtwmomThread3(&SquareCylinder3DStaggeredFieldF::CalcdphidtWmomDiv, &fdcolfield, std::ref(fdcolfield.cdWdt3),
			std::ref(aa2), std::ref(bb2), std::ref(cc2));
